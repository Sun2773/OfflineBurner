...
#define TIMESTAMP_CLOCK         SystemCoreClock ///< Timestamp clock in Hz (0 = timestamps not supported).
...
// SWDIO Pin
#define SWD_SWDIO_I_PORT  GPIOB
#define SWD_SWDIO_I_PIN   GPIO_Pin_14
#define SWD_SWDIO_I_BIT   14

#define SWD_SWDIO_O_PORT  GPIOB
#define SWD_SWDIO_O_PIN   GPIO_Pin_12
#define SWD_SWDIO_O_BIT   12

// SWCLK Pin
#define SWD_SWCLK_PORT    GPIOB
#define SWD_SWCLK_PIN     GPIO_Pin_13
#define SWD_SWCLK_BIT     13

// nRESET Pin
#define SWD_nRESET_PORT   GPIOB
#define SWD_nRESET_PIN    GPIO_Pin_0
#define SWD_nRESET_BIT    0

#define GPIO_MODE_CLR(bit)  &= ~(0xF << ((bit) * 4))
#define GPIO_MODE_OUT(bit)  |= (0x3 << ((bit) * 4))
#define GPIO_MODE_IN(bit)   |= (0x4 << ((bit) * 4))
#define GPIO_MODE_HZ(bit)   |= (0x4 << ((bit) * 4))

#define GPIO_OUTPUT_H(p)    SWD_##p##_PORT->BSRR = SWD_##p##_PIN
#define GPIO_OUTPUT_L(p)    SWD_##p##_PORT->BRR  = SWD_##p##_PIN
#define GPIO_INPUT(p)       ((SWD_##p##_PORT->IDR & SWD_##p##_PIN) != 0)
...
__STATIC_INLINE void PORT_SWD_SETUP (void) {
  ...
  SWD_SWCLK_PORT->CRL GPIO_MODE_OUT(SWD_SWCLK_BIT);
#else
  SWD_SWCLK_PORT->CRH GPIO_MODE_CLR(SWD_SWCLK_BIT - 8);
  SWD_SWCLK_PORT->CRH GPIO_MODE_OUT(SWD_SWCLK_BIT - 8);
#endif

  /* 配置SWDIO的输出为推挽输出，并输出高电平 */
  GPIO_OUTPUT_H(SWDIO_O);
#if(SWD_SWDIO_O_BIT < 8)
  SWD_SWDIO_O_PORT->CRL GPIO_MODE_CLR(SWD_SWDIO_O_BIT);
  SWD_SWDIO_O_PORT->CRL GPIO_MODE_OUT(SWD_SWDIO_O_BIT);
#else
  SWD_SWDIO_O_PORT->CRH GPIO_MODE_CLR(SWD_SWDIO_O_BIT - 8);
...
#else
  SWD_SWDIO_I_PORT->CRH GPIO_MODE_CLR(SWD_SWDIO_I_BIT - 8);
  SWD_SWDIO_I_PORT->CRH GPIO_MODE_IN(SWD_SWDIO_I_BIT - 8);
#endif

  /* 配置nRESET为推挽输出，并输出低电平 */
  GPIO_OUTPUT_L(nRESET);
#if(SWD_nRESET_BIT < 8)
  SWD_nRESET_PORT->CRL GPIO_MODE_CLR(SWD_nRESET_BIT);
  SWD_nRESET_PORT->CRL GPIO_MODE_OUT(SWD_nRESET_BIT);
#else
...
#else
  SWD_nRESET_PORT->CRH GPIO_MODE_CLR(SWD_nRESET_BIT - 8);
  SWD_nRESET_PORT->CRH GPIO_MODE_HZ(SWD_nRESET_BIT - 8);
#endif
}
...
__STATIC_FORCEINLINE uint32_t PIN_nRESET_IN  (void) {
  return (SWD_nRESET_PORT->IDR & SWD_nRESET_PIN) ? 1 : 0;
}
